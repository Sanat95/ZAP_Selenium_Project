...
// ---------------------------------------------------------------------------------------------------------
  ...
  //  setAlert_AttackStrength();
    ...
    public void method11() throws Exception {
    	...
    	String API_KEY = "stsjdiqigrcnqtaulj7691146o";
    	//String absolutePath = "/path/to/save/report.xhtml";
    	String absolutePath ="C:\\Users\\User\\eclipse-workspace\\AttraProject\\report.xhtml";
    	...
    	mp.put("apikey",API_KEY);

    	// String representing the absolute path to save destination for the report
    	mp.put("absolutePath",absolutePath);

    	// String representing the extension of the report
    	mp.put("fileExtension", extension);

    	// String separated by ; for each entry
    	mp.put("sourceDetails", sourceDetails);

    	// String separated by ; for each entry
    	mp.put("alertSeverity", alertSeverity);

    	//String separated by ; for each entry
    	mp.put("alertDetails", alertDetails);

    	ClientApi clientApi = null;
    	ApiResponseElement val = (ApiResponseElement) clientApi.callApi("exportreport", "action", "generate", mp);
    	} 
